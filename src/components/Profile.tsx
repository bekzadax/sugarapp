    ...
    if (file) {
      ...
      reader.onload = () => {
        ...
        img.onload = () => {
          ...
          if (!ctx) {
            setEditedProfile((prev) => ({ ...prev, photo: reader.result as string }));
            return;
          }
          ctx.drawImage(img, 0, 0, canvas.width, canvas.height);
          const compressed = canvas.toDataURL('image/jpeg', 0.82);
          setEditedProfile((prev) => ({ ...prev, photo: compressed }));
        };
        img.onerror = () => {
          setEditedProfile((prev) => ({ ...prev, photo: reader.result as string }));
        };
    ...
    <div className="h-full overflow-y-auto p-4">
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-5 sm:p-6">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              ...
              <div className="relative">
                <div
                  className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center text-xl sm:text-2xl text-white font-bold overflow-hidden"
                  style={
                ...
                {isEditing && (
                  <button
                    ...
                    className="absolute -bottom-1 -right-1 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-pink-500 text-white flex items-center justify-center shadow-lg"
                  >
              ...
              <div className="w-full">
                {isEditing ? (
                  <input
                    ...
                    className="w-full max-w-[220px] sm:max-w-none text-lg sm:text-xl font-bold text-slate-800 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1"
                    placeholder="@name"
                  />
                ) : (
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 mt-2">
                  {isEditing ? (
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                      <button
                        ...
                        className={`w-full sm:w-auto px-2.5 py-1 rounded-full text-xs font-semibold border transition-colors ${
                  ...
                  {isEditing ? (
                    <input
                      ...
                      className="w-full sm:w-16 text-sm bg-slate-50 border border-slate-200 rounded px-2 py-2 sm:py-0.5"
                    />
                  ) : (
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 flex flex-col md:flex-row md:items-center md:justify-end gap-3">
            <button
              onClick={isEditing ? handleSave : handleEdit}
              className={`w-full md:w-auto px-5 py-3 rounded-full text-sm font-semibold flex items-center justify-center gap-2 transition-colors ${
                isEditing
